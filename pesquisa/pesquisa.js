	...
	for (i = 0; i < lista.results.length; i++) {
		...
		switch (registro.media_type) {
			case "movie":
				...
				texto += `
					<a href="/detalhes/filme.html?id=${registro.id}" target="_blank" class="list-group-item list-group-item-action flex-column align-items-start">
						<div class="row">
							<div class="col-6">
	...
	for (i = 1; i <= lista.total_pages; i++) {
		paginas += `
			<li class="page-item ${lista.page==i?"disabled":""}">
				<a class="page-link text-dark" href="/pesquisa/pesquisa.html?query=${query}&page=${i}">${i}</a>
			</li>
		`;
	}
