const API_KEY  = '01abc0837242a7067334b817a836455a';
...
function voltarParaHome(){
	window.location.replace('/Trabalho_cinema/home/index.html')
}
	...
	for (i = 0; i < lista.results.length; i++) {
		...
		switch (registro.media_type) {
			case "movie":
				...
				texto += `
					<a href="/Trabalho_cinema/detalhes/filme.html?id=${registro.id}" target="_blank" class="list-group-item list-group-item-action flex-column align-items-start">
						<div class="row">
							<div class="col-6">
								<img class="img_pesquisas" src="${registro.poster_path?BASE_IMG_URL+registro.poster_path:DEFAULT_IMAGE}">
							</div>
							<div class="col-6">	
			...
			case "person":
				...
				texto += `
					<a href="${BASE_SITE_URL}person/${registro.id}" target="_blank" class="list-group-item list-group-item-action flex-column align-items-start">
						<div class="row">
							<div class="col-6">
								<img class="img_pesquisas" src="${registro.profile_path?BASE_IMG_URL+registro.profile_path:DEFAULT_PROFILE}">
							</div>
							<div class="col-6">	
								<div class="d-flex w-100 justify-content-between">
									<h5 class="mb-1">${registro.name}</h5>
	...
	for (i = 1; i <= lista.total_pages; i++) {
		paginas += `
			<li class="page-item ${lista.page==i?"disabled":""}">
				<a class="page-link text-dark" href="/Trabalho_cinema/pesquisa/pesquisa.html?query=${query}&page=${i}">${i}</a>
			</li>
		`;
	}
...
searchButton.addEventListener("click" , (e) => {
	window.location.href = `/Trabalho_cinema/pesquisa/pesquisa.html?query=${searchBar.value}`;
	e.preventDefault();
} )
